...
// 'https://youtube.com/playlist?list=PLzwWSJNcZTMRB9ILve6fEIS_OHGrV5R2o';
  ...
  group('Search icon button test', () {
    group('Audio selection', () {
      ...
      testWidgets(
          '''Click on dropDown button after having started to search word
          on audio. First, select the existing 'janco' sort/filter parms in the
          SF dropdown button. Then enter the search word 'La' in the 'Youtube''',
          (WidgetTester tester) async {
        // Link or Search' text field. After entering 'L', verify that the search
        // icon button is now enabled. Then, enter 'La' search word, click on the
        // enabled search icon button and verify the reduced displayed audio list.
        // After that, tap on the playlists toggle button. Then delete the 'a'
        // letter from the 'La' search word and verify that the change was not
        // applied to audio list since the playlists list is displayed. Now retap
        // on playlists toggle button and modify the search text which now applies
        // on the audio list.
        await IntegrationTestUtil.initializeApplicationAndSelectPlaylist(
          tester: tester,
          savedTestDataDirName: 'sort_and_filter_audio_dialog_widget_test',
        ...
        // Now tap on the 'Toggle List' button to display the playlists list
        await tester.tap(find.byKey(const Key('playlist_toggle_button')));
        await tester.pumpAndSettle();
